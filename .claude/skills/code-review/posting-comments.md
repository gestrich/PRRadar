...
See: https://github.com/jeppesen-foreflight/ff-ios/blob/develop/FFMDevGuide/Sources/FFMDevGuide/Documentation.docc/ObjectiveC/Nullability/Nullability.md
```
...
If approved, run:
```bash
cat << 'EOF' | gh api repos/jeppesen-foreflight/ff-ios/pulls/18615/reviews --method POST --input -
{
  ...
  "comments": [
    {
      "path": "ffm/libraries/FlightPlanning/Sources/FlightPlanning/Procedure Preview/ProcedurePreviewViewController.m",
      "line": 63,
      "side": "RIGHT",
      "body": "This property should have a nullability annotation. Given that it's set to `nil` after use, it should be `nullable`:\n\n```objective-c\n@property (nullable) UIActivityIndicatorView *loadingSpinner;\n```\n\nSee [FFMDevGuide for Nullability](https://github.com/jeppesen-foreflight/ff-ios/blob/develop/FFMDevGuide/Sources/FFMDevGuide/Documentation.docc/ObjectiveC/Nullability/Nullability.md)"
    }
  ]
}
EOF
