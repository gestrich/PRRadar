...
struct SaveConfigurationUseCaseTests {
    ...
    func addFirstConfigBecomesDefault() throws {
        ...
        let config = RepositoryConfigurationJSON(name: "first", repoPath: "/tmp/repo", githubAccount: "test", defaultBaseBranch: "main")
    ...
    func addPersistsToDisk() throws {
        let service = makeTempService()
        let useCase = SaveConfigurationUseCase(settingsService: service)
        let config = RepositoryConfigurationJSON(name: "persisted", repoPath: "/tmp/repo", githubAccount: "test", defaultBaseBranch: "main")
    ...
    func updateExistingConfig() throws {
        ...
        let useCase = SaveConfigurationUseCase(settingsService: service)
        let configId = UUID()
        let original = RepositoryConfigurationJSON(id: configId, name: "original", repoPath: "/tmp/repo", githubAccount: "test", defaultBaseBranch: "main")
    ...
    func saveUnknownIdAdds() throws {
        let service = makeTempService()
        let useCase = SaveConfigurationUseCase(settingsService: service)
        let config = RepositoryConfigurationJSON(name: "new", repoPath: "/tmp/repo", githubAccount: "test", defaultBaseBranch: "main")
