...
## - [ ] Phase 9: Create `PRReviewRequest` and deduplicate `PRReviewResult` / `PRReviewSummary`

**Skills to read**: `swift-app-architecture:swift-architecture`

Two changes:

**9a: Create `PRReviewRequest`**
- Create `PRReviewRequest.swift` in `PRReviewFeature/models/` with:
  ```swift
  ...
  ```
- Update `AnalyzeUseCase.execute()` to accept `PRReviewRequest` instead of loose parameters
- Remove the unused `repoPath` parameter
- Update callers: `RunPipelineUseCase`, `PRModel`, `AnalyzeCommand`

**9b: Deduplicate `results` array**
- `PRReviewSummary` currently holds `results: [RuleOutcome]`, duplicating what's in `PRReviewResult.evaluations`
- Remove `results` from `PRReviewSummary` — it becomes a pure stats struct
- Move the `modelsUsed` computed property to `PRReviewResult` (it can derive from `evaluations`)
- Update all code that reads `summary.results` to use `PRReviewResult.evaluations` instead
- No backward compatibility needed for JSON format changes — old data will be deleted
- `swift build` and `swift test` to verify
