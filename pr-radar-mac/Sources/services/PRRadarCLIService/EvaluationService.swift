...
private let evaluationPromptTemplate = """
...
nonisolated(unsafe) private let evaluationOutputSchema: [String: Any] = [
    "type": "object",
    "properties": [
        "violates_rule": [
            "type": "boolean",
...
public struct EvaluationService: Sendable {
    ...
    public func evaluateTask(_ task: EvaluationTaskOutput, repoPath: String) async throws -> RuleEvaluationResult {
        let model = task.rule.model ?? defaultModel
        let focusedContent = task.focusArea.getFocusedContent()

        let prompt = evaluationPromptTemplate
